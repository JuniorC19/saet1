$(document).ready(function(){
	...
	var table = $('#table-data').DataTable({
		...
		ajax:{
			url:'/extranet/configuracion/categoria?datatable=true'
		},
		columns:[
	    ...
	    rowCallback: function( row, data, index ) {
	    	...
	    	$('td:eq(0)', row).delegate('.editar','click',function(e){ 
				...
				editar = 1;
				// alert(href);
				$("#title-datatable").text("Editar Categoria");
				$.ajax({
                    type: "GET",
                    dataType: "json",
                    url:"/extranet/configuracion/categoria/"+href+"/edit",
	...
	$("#nuevo").on("click",function(){
    	editar = 0;
    	$("#title-datatable").text("Nueva Categoria");
    	$("#nombre").val("");
		$('#modal-datatable').modal('show');
	});
    	...
    	if(validar == true){
    		...
    		if(editar==1){
			    $.ajax({
			        type: "PUT",
			        dataType: "json",
			        url: "/extranet/configuracion/categoria/"+href,
			        data:data,
			        success:function(data){
			          // console.log(data);
			          // load.close();
    			...
    			$.ajax({
			        type: "POST",
			        dataType: "json",
			        url: "/extranet/configuracion/categoria",
			        data:data,
			        success:function(data){
			          // console.log(data);
			          // load.close();
