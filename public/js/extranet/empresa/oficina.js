$(document).ready(function(){
	...
	var table = $('#table-data').DataTable({
		...
		ajax:{
			url:'/extranet/empresa/oficina/'+ide+'?datatable=true'
		},
		columns:[
	    ...
	    rowCallback: function( row, data, index ) {
	    	...
	    	$('td:eq(0)', row).delegate('.editar','click',function(e){ 
				href = $(this).attr('href');
				$("#title-datatable").html("Editar Oficina");
				$.ajax({
                    type: "GET",
                    dataType: "json",
                    url:"/extranet/oficina/"+href+"/edit",
	...
	$("#nuevo").on("click",function(){
    	...
    	editar = 0;
    	$("#title-datatable").text("Nueva Oficina");
    	$("#direccion").val("");
		$('#modal-datatable').modal('show');
	});
